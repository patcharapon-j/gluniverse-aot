...
| Tracker state marks | 3: `pos-hooked`, `titan-frenzy`, `titan-opening` | stamp, 128 px webp, WIDE-CAPSULE for `pos-hooked` and DIAMOND for the two `titan-` marks | **Generated and shipped, batch 1 of 2026-09-21.** Flat ink on transparent: `tracker/badges.ts` inverts the badge icon, so a filled disc becomes a pale blob. Proof at 20 px, not 32. The Frenzy 0 to 3 track and the Momentum pip row are drawn in code, not generated (ADR-0027) |
| Engagement board | 29: 15 hex terrain tiles, 5 anchor rating rim glyphs, 4 Titan figures, 2 soldier figures, 3 effect overlays | flat-top hex, shallow isometric, two-to-one foreshortening; shipped under `foundry/static/assets/board/` | Batch 2, `docs/playtest/feedback/round-3/ART-HANDOUT.md` section 4. Gated on its own style lock: the owner ruled on 2026-09-21 that the board is a new family and takes an approved style lock before the 29 files, as ADR-0022 requires of every family. Working folder `foundry/art-src/board/` |
...
Action icons: Nape strike, Fly, Dodge, Rally. Status icons: Down, Grabbed. Foe plate: bandit. Soldier portrait: Slayer. Engagement board: pending, see the board row above. Dice (locked): the four presets in `dice/final/`, shown on four rendered dice, one per die kind, each with its own baked label ink and material. Icons and plates: approved.
...
**Per-face colour mechanism.** Dice So Nice draws an image label with the colorset's `labelComposite`: `source-over` draws the image as is, `tint` recolours every image label on the die with the one `foreground` colour. Per-face colour overrides exist only for Dice Library dice, not for presets a system registers. So the ink is baked into each kind's label PNGs (`make_dice.py`, `KIND_FACES`), every colorset keeps `labelComposite: 'source-over'`, and `foreground` only matters for text fallbacks. On Gear and Stress dice the 1 face is baked in dark grey `#2E2E33`, shaded as a cut groove (top-left wall `#161619`, bottom-right wall `#55555E`), with a steep-walled bump so Dice So Nice renders it as a deep engraving; it reads as a flaw, never as a 6. (It replaced the earlier signal-flare violet `#D4A8FF` after the first playtest.)
...
| Gear | `dg` | locked | 6 Wings emblem; 1 cracked cog; 2 to 5 blank | 6: as base; 1: `originals/die-mark-gear-1-cog.png` (first cog draw) → `dice/marks/gear-1-b` | `wof-gunmetal` | `#56605F` | `metal` | none | 6: `#E6C57C` brass; 1: `#2E2E33` grooved grey |
| Stress | `ds` | locked | 6 Wings emblem; 1 fracture; 2 to 5 blank | 6: as base; 1: `originals/die-mark-stress-1-fracture.png` (first fracture draw) → `dice/marks/stress-1-b` | `wof-wax` | `#8B2A21` | `pristine` (matte base under a hard clear coat: lacquered wax) | none | 6: `#F1E4C8` parchment; 1: `#2E2E33` grooved grey |
