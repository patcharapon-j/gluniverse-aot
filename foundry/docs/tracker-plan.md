...
On the active GM's client, when the card comes up (`behavior-procedure.yaml`, resolving_a_card): dead, nothing; holding a Grabbed soldier, nothing (Next Behavior kept, flags stay); a decoy, the hold counts down, the Next Behavior becomes previous and a new one is rolled (the ladder is evaluated on the hold's last card); otherwise the ladder is evaluated (`attention.ts`: candidates, rungs top to bottom, struck-first, narrowing, holder keeps a tie, lowest card with a Wing after its player character, else nothing) and written to `attention_holder`; no holder, nothing. Then the choose step picks the entry (Body Parts, the holder's Position against the requirement, the fallback, Thrash) and its targets (the holder, or the holder and everyone at the same Position who is not Grabbed). **Nothing is rolled here.** The tracker posts a GM-only card (`behavior.ts`) naming the behavior, its tier, the Attention it answers, its targets, and the Attack Dice waiting; the table's chat note says only who holds the Attention. "Roll it" opens the Titan roll dialog, where the GM may take any entry of the table instead, tick the targets themselves, and set the dice, and choose whether Dice So Nice throws them (off by default, so a Titan's turn never rains dice on the table); "No dice" resolves the behavior with nothing thrown and announces it. Rolling posts the usual public attack card, with each dodge already made against this Titan this round written on as a Reaction, and writes it back to the Titan's row. When the card ends (the GM advances), the attack's effects are resolved if the GM has not resolved them yet, the GM card stops offering the roll, and the next step applies: previous behavior, decoys in a row 0, flags cleared, a new Next Behavior rolled and revealed after a telegraph.

The GM also overrides the hidden Next Behavior itself, from the board's Titan block or the Titan sheet. Both are deliberate table tools rather than rules: `behavior-procedure.yaml` (next_behavior, hidden; resolving_a_card, roll-and-announce, gm_choices) leaves the GM no choice at all, and the system's default remains exactly what it prescribes — the D6 picks the Next Behavior and the choose step settles the card. The dialog only lets a table that wants to call it differently do so, in the open, before any dice.
...
## 7. Zones, attachments, and derived Positions (decision batch 16, ADR-0029)

Superseded in batch E: the engagement is fought over a field of zones (`data/engagement/zones.yaml`). `combat.system.field` holds the field (size, field rating, each zone's rating, start rating, Sparse grace, and effects); `combat.system.placements` holds each soldier's zone (null off field), attachment (ground, anchored, on-body, blind-spot, grabbed, pinned, with the body it names), horse zone, and arrival stamp; each Titan row holds its `zone`. `snapshot()` derives every Position (`zones.ts`, `derivePositions`); nothing writes one, and `SoldierState.positions` is only that derivation, so the Attention Ladder, strikes, and steam read unchanged.

- Rules, pure and tested (`test/zones.test.ts` passes the shared cases of IMPLEMENTATION-PLAN-E section 5.5): `zones.ts` (layouts, distance, neighbours, edge, derivation, Momentum cap by zone, Carry costs, crossing, entry zone, wrecking one rating step with the per-zone Sparse grace, field generation with the terrain mix), `stride.ts` (route toward the holder, lower number on a tie, what moves with the Titan and what stays), `positions.ts` (`moveOptions` over zone and attachment steps for on foot, mounted pace, and a Flight with its Carry limit and no free end in an Open zone; `routeOption` checks any chosen route; leaving and returning), `retreat.ts` (`retreatOptions`, 16-27). The values come from the data through `configureZones` (`tools/config-data.ts`, `zoneRulesOf`); `ZONE_DEFAULTS` equals them and a test keeps it so.
- Engine (`engine.ts`): `requestZoneMove` (a player's request goes to the GM as `zone-move`; a Flight prompts the soldier's owner and, when answered, gains Momentum to the start zone's cap, pays its Carries, goes as far as it can pay, trims to the end zone's cap, and sets the crossing and no-success flags), `letGo`, `leave`, `returnTo`, the stride step in a Focus Titan's card between attention and choose, local wrecks with zone-becomes-open, the entry zone of a promoted Titan, and the Direct Control setters `gmPlace`, `gmPlaceTitan`, `gmPlaceHorse`, `setZoneRating` (each a ruling, undoable). `combat.system.boardEvent` (`stride`, `flight`, `wreck`, `fall`, `steam`, `enter`) is written in the same update as the change; its `seq` only rises and Undo never writes it back.
- Ledger: the left page's field readout lists each zone's rating and occupants, and each soldier's zone, attachment, airborne, mounted, horse, and Momentum against their zone's cap; derived Positions stay as read-only cells with let go, Draw Attention, and the Direct Control placement setters. Moves are made on the engagement board (`src/board/`).
