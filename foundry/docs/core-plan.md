...
foundry/
  ...
  static/                copied as-is into dist/
    ...
    assets/icons/        stamp icons (site icons, Action Catalog, token statuses)
    assets/portraits/    (2e) default Soldier portraits, one per Specialty
    assets/plates/       Titan and Foe plates, the setup background
    assets/dice/         (2e) Dice So Nice labels, bump maps, and textures
  ...
  src/
    ...
    art.ts               (2e) shipped art paths, default artwork, the token status list
    ...
    documents/actor.ts   (2e) WofActor (bound token statuses, default artwork), WofTokenHUD
    ...
    dice/                (2d) terms.ts (DiceTerms, WofRoll), actor-pool.ts, roll-action.ts, roll-dialog.ts,
                         ...
                         apply.ts (ops), proxy.ts (GM queries), post.ts, dsn.ts (2e, Dice So Nice presets)
    motion/              (2b) motion tokens, anime.js wrappers; (2e) widgets.ts, the one three.js renderer
    settings.svelte.ts   (2d, 2e) world and client settings
    settings-menu.ts     (2e) the preferences menu (Motion, Gore, GM auto-apply)
  tools/
    ...
    import-art.sh        (2e) copies approved art from art-src/ into static/assets (run by hand, needs cwebp)
...
`foundry/design/` and `foundry/art-src/` stay design inputs; the build never reads them. `tools/import-art.sh` copies the approved art into `static/assets` by hand.
...
11. **Dice So Nice faces.** Decided (2e): labels ship as lossless webp (alpha kept exactly), bump maps as greyscale PNG, identical faces stored once (`static/assets/dice/`). The kind presets are registered under the system's own dice set ("preferred") and, because each replaces the stand-in Dice So Nice adds for a custom DiceTerm, they keep their faces and colorsets whichever set a player picks. The four kind colorsets are hidden from the player's theme list.
12. **One WebGL renderer.** Decided (2e): the widgets share one `WebGLRenderer` that is never attached to the page; each widget copies its corner of the renderer's buffer into its own 2D canvas with `drawImage`. The renderer is created with the first widget and disposed (context released) when the last one unmounts, so a client holds at most one widget context, and none with no Soldier sheet open. The compact Squadmate sheet shows no widgets.
13. **Token statuses.** Decided (2e): `CONFIG.statusEffects` holds the 13 system statuses plus core's Dead. Statuses the model records are bound to it (`src/rules/statuses.ts`): Down, Pinned, Airborne (and a Foe's Held) toggle their field from the Token HUD; Untreated Critical Injury, Jammed, Overloaded, Mounted, Lame horse, Carrying, and Carried follow the sheet and the HUD refuses them with a note; Grabbed, Engaged, and a soldier's Held stay ordinary Active Effects until the tracker (milestone 4). Bound statuses are drawn on the token as unsaved effects and added to `actor.statuses`.
14. **Default art.** Decided (2e): `WofActor.getDefaultArtwork` gives a Foe its plate (the bandit's for an unknown kind) and a Titan its Size Class plate and stamp; a Soldier or Squadmate keeps Foundry's placeholder until it gets a Specialty, then takes that Specialty's portrait (also when its image is another Specialty's portrait). The sheet plate shows the Corps emblem for a placeholder. Origins use the emblem in the pack until 2f.
15. **Auto-apply switches.** Decided (2e): the five GM switches are listed in the preferences menu only, not one by one in Configure Settings; Motion and Gore are in both.

## 9. Status

**2e done (2026-09-16).** **Dice So Nice** (`src/dice/dsn.ts`): on `diceSoNiceReady` the system registers its dice set, the bone and flesh textures (multiply, with bump maps), one hidden colorset per kind (`wof-bone` plastic, `wof-gunmetal` metal, `wof-wax` pristine, `wof-flesh` pristine, label composite source-over), and one d6 preset per kind (`db`, `dg`, `ds`, `dt`) with six labels and six bump maps (marked faces per the locked table, the rest blank), then preloads them; checked against the Dice So Nice 6.3.0 source (`addDicePreset` colorset field, the standard-set replacement of automatic presets). A Push shows only the re-rolled dice (unchanged from 2d). **Widgets** (`src/motion/widgets.ts`, `Widget3d.svelte`, `Vitals.svelte`): the locked preview's gas canisters (fitted canister with a band per point of the full Gas Rating, up to three spares with their own levels, dull when there is no ODM Gear or it is Jammed) and blade sets (the set in the handles, up to four carried sets, snapped stubs after a ruin), painted on a value change; Full motion jolts and flashes on a gas spend, drops the new canister in on a change, drops and fades the blades on a ruin, and slides a set in on a swap; Reduced keeps the colour flash; Off, or a client without WebGL, draws the preview's still SVGs. **Preferences menu** (`src/settings-menu.ts`): Motion and Gore as stamped choices with a line each, the OS reduced-motion note, and for the GM the five auto-apply switches; Save writes only what changed; Motion and Gore stay in Configure Settings. **Art** (`tools/import-art.sh`, `src/art.ts`): 46 Action Catalog icons (quick-roll rows, the roll dialog, and a new stamp tile in every action, Fear, Stress Response, and Gas card header), 13 token status icons at 256 px, nine Specialty portraits, three Foe plates, the setup background as the system's `background` and setup media; pack entries carry their images (Talent dice or rule stamp, Specialty stamp, gear stamp, injury stamp, Squadmate portraits, Foe plates, Titan plates). **Token statuses** (`src/documents/actor.ts`, `src/rules/statuses.ts`): decision 13. **Card header**: the name keeps its width and the pool glyphs give way first; "Nape strike", "Body Part strike", "Break Attention", "Field Repair", and "Leap Clear" fit with the Pushed flag. `pnpm test`: 99 tests (10 new: shipped art, presets, bound statuses, pack images). `pnpm build`, `pnpm typecheck`, `svelte-check` clean. Browser check on the isolated 14.365 server (port 30055) with Dice So Nice 6.3.0 installed only in the scratch data folder: each kind rolled in its colorset and material with its marked faces (violet 1s on Gear and Stress, fang ring on Titan 5 and 6, veined flesh), a pushed Nape strike showed only its 5 base and 3 Stress Dice; frame strips of the gas spend and change and the blade ruin and swap; Reduced and Off paths; the menu saved Motion Off (sheet switched to stills and back to one renderer live), Gore, and a GM switch; the 13 statuses in the Token HUD with bound ones lit, their icons on a token, Down toggled through its field, Jammed refused, a treated injury clearing its icon; a new Soldier took the Medic portrait on its Specialty, new Foes their plates, a new Titan its plate and stamp; the setup screen showed the plate; four sheets open held one widget context, and five open and close cycles of five sheets never held more than one and left none live; no console errors. Screenshots in the agent's scratchpad (`e2/`). Deferred: Origins have no own icon (2f); Grabbed, Engaged, and a soldier's Held are manual until the tracker; a released widget context stays in memory, lost, until the browser collects it.
