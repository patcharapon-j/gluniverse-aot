...
| resolve | ceil((instinct + empathy) / 2) + Scars - min(Grief, 3) | derived_values.resolve |
...
1. **Resolve floor.** The formula can go below 0 (Instinct 2, Empathy 2, Grief 3). The data sets no floor; the model shows the formula's value. Does Resolve stop at 0?
2. **Gas canister.** ADR-0025's gear subtypes have no canister, so the fitted canister and the spares are fields on the actor (`gas_rating`, `spare_canisters`) and the gear pack has no canister entry. Keep it that way?
3. **Critical Injury pack.** One entry per row (31), named by location, 2D6 range, and Crush name with the other names listed; the Injury Type is picked when the injury is taken. Or five entries per row (155)?
