<script lang="ts">
  /**
   * The compact Squadmate sheet (core-plan 2c): the stat block of data/character/squadmates.yaml,
   * in the Soldier's Dossier frame (sheet-overhaul plan, 12.1): the slim header, the vitals band,
   * the index tabs on the right edge and the footer on the binder (FileFrame), reusing the
   * Soldier's Wounds & Mind tab and Kit ledger.
   */
  ...
  import { squadTags } from '../header-tags.ts';
  ...
  const TABS = [
    { id: 'stat', label: 'WOF.Squad.tab.stat', tc: 'var(--red)' },
  ...
  const tags = $derived(squadTags(view, wingName));
...
<FileFrame
  ...
  mode={view.mode}
  tabs={TABS}
  tabLabel={t('WOF.Squad.tab.label')}
  footLeft={t('WOF.Squad.footLeft')}
  footRight={t('WOF.Sheet.foot.right')}
  cls="squad"
>
  {#snippet top()}
    <Header {view} kicker={t('WOF.Squad.kicker')} nameLabel={t('WOF.Squad.name')} nameDisabled={roStats} {tags} {serial} {meta} />
    <Vitals {view} compact />
  {/snippet}
  {#snippet page(tab)}
    {#if tab === 'wounds'}
...
{#snippet meta()}
  <span use:contextMenu={view.specialty ? itemMenu(view.specialty.id, true) : () => []} use:dragItem={view.specialty ? { item: actor.items.get(view.specialty.id) } : null}>
