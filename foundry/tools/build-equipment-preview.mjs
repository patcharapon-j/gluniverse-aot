...
const names=['EquipmentRig','Dots','HealthTrack','StressTrack','Vitals','Header','Plate','ModeSwitch','TokenButton'];
const stub=`
...
const tokenStudioActive=()=>false;
const browseImage=()=>{}, openTokenStudio=()=>{}, openPrototypeToken=()=>{}, resetImage=()=>{}, saveUploadedImage=async()=>{};
...
setContext('equipment-proof',{actor,state:{},sheet:{setSheetMode(mode){view.mode=mode;view.statsEditable=view.editable&&mode==='edit'}},uid:'proof'});
globalThis.proof={derived(data){Object.assign(view.derived,data)},compact(v){compact=v},reset(){view=make()},set(data){for(const [k,v] of Object.entries(data))view.system[k]=v},readonly(v){view.editable=!v;view.statsEditable=!v&&view.mode==='edit'},state(){return JSON.parse(JSON.stringify(view))},motion(v){mode=v},width(v){width=v}};
</script>
...
await build({entryPoints:['.proof-entry.js'],bundle:true,conditions:['browser'],format:'iife',supported:{'template-literal':false},outfile:'design/equipment-preview.js',minify:true});
