...
- **Not taken:** a Talent naming `draw-attention`, which would have made it no longer the only Titan Engagement action with none. Each Specialty's Talent list is exactly eight and the total is 83, both schema-enforced, so adding one means dropping one. Deferred to the Phase 2 Talent pass.
