...
### OQ-182: ODM movement, the environment, and what a flight is worth

- **Type:** Design gap (owner feedback round 2: ODM movement and the environment read flat)
- **Arose in:** `docs/playtest/feedback/round-2/odm-movement-feel.md`, `design-proposals.md`, and `unified-design.md`; `docs/playtest/feedback/round-2/flatness-audit.md` findings 1, 3 and 5. Chapter 4 sections 4.2, 4.3; Chapter 5 sections 5.1, 5.2, 5.3, 5.7, 5.13; `data/engagement/anchor-ratings.yaml`, `positions.yaml`, `round.yaml`, `engagement-setup.yaml`, `titan-format.yaml`, `titan-harm.yaml`; `data/gear/odm-gear.yaml`, `sheet-fields.yaml`; `data/core/bonus-dice-sources.yaml`; `data/character/action-catalog.yaml`; `data/titans/`.
- **Related:** OQ-74, OQ-75, OQ-99, OQ-183, OQ-184, OQ-185.
- **Question:** A move in a Titan Engagement was never rolled, never risked, and never paid for. Measured: of 20 Position step rows only 2 called for a Fly roll, and neither was at Wooded, the default battlefield; both failed back to exactly where the safe step would have landed the soldier, so the roll staked no Position; a canister lasts a median 8 rounds of ODM use against a 3.91 round fight, and the reference Squad made 0.57 Gas Rolls a round across four soldiers. The Anchor Rating's only job was to set how fast the Squad reached the Nape, monotonically: Giant Forest gave Blind Spot on round 1 free and guaranteed, Wooded on round 2, Open never while the Titan stands (`tools/probes/round-2/swing.out`). Wirework, a dice Talent to level 3 naming only `fly`, was live on 2 of 20 rows.
- **Options:** (a) Leave it. (b) A free two-step move at every rating: measured to move round 1 arrival at Wooded from 0.0% to 66.2% and the mean approach from 2.00 to 1.34 rounds, so it shortens the fight and adds no decision. (c) Anchors alone: a finite terrain resource spent for a two-step Swing. (d) Momentum alone: an ODM move builds a currency, lost the moment a soldier stops flying. (e) An Approach Roll alone: every flight rolled, extra successes spent on a menu the terrain offers. (f) The three merged: the roll produces the currency and the terrain caps it.
- **Status:** Decided (see DECISIONS-2026-09-14.md, *Batch 10*, 10-1)
- **Decision:** (f), the merge. Every ODM move is a **Flight**, rolled for `fly`; the step happens whatever the roll gives, each success is 1 **Momentum**, and no successes sets the loudest flag from any Position. Momentum is capped by the **Anchors** left, is lost at the end of any round in which the soldier made no ODM move, and is spent on Carry, Bite, Brace, Quiet, or a clean line. Anchors are set by the Anchor Rating (Open 0, Sparse 1, Wooded 2, Urban 3, Giant Forest 3), are never restored, and are destroyed by the new `wreck` effect and by a falling Titan. Each rating gains a Terrain Trait; Wooded gains none, so the tuned default is untouched by that piece. The two Fly roll step rows and Giant Forest's Distant to Blind Spot row are withdrawn, so no rating reaches the Nape from Distant in one step and the ratings now differ by Anchors and Trait rather than by speed.
- **ADR:** ADR-0010 is untouched: no Attention rung moves and no penalty for acting alone is added. ADR-0024 limit 8 holds: every value is printed and the GM never picks a step, an Anchor, or a point of Momentum. ADR-0014: every number here is a starting value.
- **Simulator case:** required, and this is a retune rather than a sensitivity row, because Flight replaces the move rather than sitting beside it. The rerun must answer: the kill-round target (`prepared_squad_kill`, median exactly 3), with **Bite** measured separately as the first thing to cut; the PC Critical Injury and death targets, which the loudest flag on a failed Flight feeds alongside OQ-184; the wreck rate per table and Size Class; and `families.route_check` with Anchors at 0, which must confirm ADR-0010's legal route still holds. Round time rises by about 1 to 1.5 dice pools a round against today's 3.57.

### OQ-183: A step of the horse's own

- **Type:** Design gap (owner feedback round 2)
- **Arose in:** `docs/playtest/feedback/round-2/flatness-audit.md` finding 1; `design-proposals.md` fix 2. `data/engagement/anchor-ratings.yaml`; `data/gear/horses.yaml`; Chapter 5 section 5.2.
- **Related:** OQ-74, OQ-182.
- **Question:** Enumerating all 20 Position step rows, there was not one step a mounted move could make that a move on foot could not: every row with `mounted: true` also had `on_foot: true`. A horse's whole value inside a Titan Engagement was its Gear Dice on the dodge and Break Attention plus the riderless-horse decoy, so "mounted" was a named movement mode with no movement of its own, and the Rider Specialty carried an unfunded promise.
- **Options:** (a) Accept it: the horse is Gear Dice and a decoy. (b) Give the horse a step nothing else has. (c) Give the horse an act nothing else has.
- **Status:** Decided (see DECISIONS-2026-09-14.md, *Batch 10*, 10-2)
- **Decision:** (c) with a piece of (b). A mounted move that makes the Distant to In Reach step may also set the **loudest** flag, the **charge**: no other move sets a flag, so it is the horse's own capability, and it gives the Rider the bait role Loose the Horse and Cavalry Cut already gesture at. At the **Open** rating only, a mounted move may make that step twice, in either order, so a rider can come in and get out again in one move. It is one move and one charge. The charge is never rolled, spends nothing beyond the move, and is not ODM use.
- **ADR:** none. The charge writes an existing flag by an existing rule and adds no rung.
- **Simulator case:** required, in the same rerun as OQ-182 and OQ-184, since all three write the loudest flag.

### OQ-184: What a shout can take a Titan off

- **Type:** Rules gap (Draw Attention measured as actively harmful)
- **Arose in:** `docs/playtest/feedback/round-2/flatness-audit.md` finding 2; `design-proposals.md` fix 1. `docs/reviews/simulator-report.md` sections 6.6 and 6.7. `data/engagement/attention.yaml`; `data/character/action-catalog.yaml`; Chapter 5 section 5.6.
- **Related:** OQ-81, OQ-113, OQ-182, OQ-183.
- **Question:** Draw Attention set the loudest flag, which is rung 4 of the standard ladder, while standing at In Reach is rung 2 and free and being hooked in is rung 1. So the action changed the outcome only as a tiebreak among soldiers already meeting the same higher rung, and it could not take a Titan off the comrade a player most wants to save. The simulator measured taking it at Critical Injuries z +9.7 and deaths through the end z +2.5, with no faster kill, and counted 0.49 attempts a fight barred because the soldier was at Distant. It was also the only action in a Titan Engagement that no Talent names.
- **Options:** (a) Wording only: print plainly what the action can and cannot do. (b) Give the flag reach: while a soldier holds it they meet every rung any other candidate meets except hooked-into-its-body. (c) Give the drawer a defensive benefit instead.
- **Status:** Decided (see DECISIONS-2026-09-14.md, *Batch 10*, 10-3)
- **Decision:** (b). While a soldier holds the loudest flag they also count, for that Titan's ladder, as meeting every rung any other candidate meets **except hooked-into-its-body**. A new evaluation step, `loudest-matches`, sits between `top` and `struck-first` and is the only step that adds a candidate; it adds them whatever Position they hold, so a soldier who came in loud from Distant is in the set. The shout therefore takes a Titan off any comrade who is not hooked into its body or On Body, and never off a blade at its Nape. This is exactly the room ADR-0010 leaves, whose promise is "no rung, noise included, turns a Titan off a blade at its Nape or a soldier On Body".
- **ADR:** ADR-0010 is preserved as written, not amended: rung 1 is untouched and the promise it makes is the boundary the fix respects.
- **Simulator case:** required. It makes the fight safer for strikers and more dangerous for the drawer, so it moves the PC Critical Injury and death targets in both directions and the net is not guessable. Measure it together with OQ-182's failed-Flight flag and OQ-183's charge, since all three write the same flag.
- **Also decided (owner, 2026-09-17):** the fixed list size is removed and the Talent is added. `Shout Them Off` (Leader, rule, once per Titan Engagement): you declare Draw Attention in a Titan Engagement, and you may spend your move on it instead of your action. It follows Quick Refit's precedent exactly, so it needs no new machinery: no Talent in the game grants Bonus Dice and there is no Talent row in the closed Bonus Dice source list, so a cost swap is the shape a rule Talent takes here. Draw Attention is no longer the only Titan Engagement action no Talent names. `specialties.yaml` `list_size` now reads "at least eight", the Leader's list holds nine, the total is 84, and the schema that enforced exactly eight is loosened to a minimum.

### OQ-185: What a ruined Blade Set costs

- **Type:** Design gap (owner feedback round 2: blades cannot run out)
- **Arose in:** `docs/playtest/feedback/round-2/flatness-audit.md` finding 3; `unified-design.md`, *The Gear Dice question*. `data/gear/blade-sets.yaml`; `data/character/action-catalog.yaml`; Chapter 4 section 4.4.
- **Related:** OQ-63, OQ-01, OQ-12, OQ-182.
- **Question:** A Blade Set is ruined only by wear from a Pushed roll, at 1/6 per Pushed strike on an issued rating 1 set, while Swap Blade Set spent neither the move nor the action and a Funding 3 Rookie carries three sets. Against a 3.91 round fight in which a striker makes two or three strikes, blades could not run out, and the most repeated image in the source was unreachable. The owner proposed re-rolling Gear Dice on a Push as the fix.
- **Options:** (a) Leave it. (b) Re-roll Gear Dice on a Push, in the Stress Die's pattern (re-roll 2 to 5, keep 6, lock 1). (c) Make the swap cost the move. (d) (b) and (c) together.
- **Status:** Decided (see DECISIONS-2026-09-14.md, *Batch 10*, 10-4)
- **Decision:** (c) now. In a Titan Engagement, Swap Blade Set spends the soldier's **move**; outside one it still spends nothing. A move can change no Position, so a soldier who swaps stays where they are and still has their action: what a ruined Blade Set costs is that turn's Flight, and with it the Momentum that Flight would have given. It touches no die and no tuned number, and it does not close ADR-0010's route, since a soldier can refit with their move and strike with their action on the same turn.
- **Not taken now:** (b). Measured (`tools/probes/round-2/gear_die.out`): it moves a Gear Die's 1 and its 6 both from 16.7% to 27.8%, which is a good trade for the Push economy, but it takes blades only from one ruin every 4 fights to one every 2.4, so it is not a blade fix; and it takes the Jam test from 26.3% to 53.1% against a bar of no more than a third. A higher issued rating does not rescue it, because a higher rating rolls more Gear Dice. The only combination tested that stays under the bar is the re-roll plus a wear cap of 1 per Pushed roll plus issued ODM Gear at rating 3, at 24.2%: three tuned values moving together, including a Funding-table change, touching every Pushed roll with a gear item. Deferred to the post-playtest retune package that batch 7's item 7 already schedules.
- **ADR:** ADR-0004 is untouched by the decision taken. The deferred option would amend it.
- **Simulator case:** required for (c), which the simulator already models (Blade Set ruin is in the full-fight list), but no tuned value moves, so it is the cheapest kind of rerun. Measure it in the same run as OQ-182.

### OQ-186: Whether a forced step and a retreat's move are Flights

- **Type:** Rules gap (raised by the packet drafter while applying decision batch 10, 2026-09-17)
- **Arose in:** applying 10-1 to `docs/playtest/wings-of-freedom-playtest-packet.html`. `data/engagement/positions.yaml` (`moves`, `forced_step`); `data/engagement/background-titans.yaml` (`retreat`, `moves`); `data/harm/effect-types.yaml` (`forced-move`).
- **Related:** OQ-182, OQ-119, OQ-129; decision batch 7, 7-8; decision batch 8, 8-13.
- **Question:** Batch 10 made every ODM move a Flight, which is rolled. Two changes of Position are not ordinary moves and the batch did not say what they are: a Fear Roll result's forced step, and a move made under a retreat. Before batch 10 a forced step was "a step of the kind your move makes, with any Fly roll it needs", which mattered only on the two step rows that called for one. With every ODM move rolled, the question is live for both, and the answers differ in whether a panicking or retreating soldier rolls, gains Momentum, and can come in loud.
- **Options:** (a) Both are Flights. (b) Neither is. (c) The retreat's move is a Flight and the forced step is not.
- **Status:** Decided (see DECISIONS-2026-09-14.md, *Batch 10*, 10-5)
- **Decision:** (c), which is what the two rules already say if they are read strictly. **A retreat narrows the soldier's own move and does not replace it**: `retreat` `moves` reads "cannot decline their move. Each move they make must be one of these", so a move under a retreat is the soldier's own move, and an ODM one is a Flight like any other, rolled, giving Momentum, and setting the loudest flag on no successes. **A Fear Roll's forced step is not the soldier's move**: `forced_step` already reads "a change of Position a rule names, not the soldier's own move", it spends nothing, it happens even on a turn spent in advance, and it never meets a Drive's own-move trigger. It is therefore never a Flight: not rolled, no Momentum, no loudest flag, whichever kind of step it is. An ODM forced step is still ODM use, so it makes that round's Gas Roll, and the soldier is airborne after it as after any ODM move.
...
- **Current handling:** Missed and logged. No value, rule, or limit changes; `data/titans/tuning.yaml` names the miss beside the floor (`targets`, `abnormals`, `missed`), and the report renders the two cells Missed (OQ-199).
- **Why still open:** The cause is a rule the owner chose (decision batch 13, 13-9) doing for the standard Medium what the runner's table never needed, and every lever touches a measured row or the bar's own definition. The decision belongs after the first playtest, with OQ-140.
- **Simulator case:** the bar's screen-with-the-pair row in both orders, with its twins, under whichever option is taken, deaths through the end and during the fight both reported.
- **Status:** Open
