...
- When a Pushed roll is final, an item whose Gear Dice show one or more 1s wears by 1 point: one point per Pushed roll, however many of its Gear Dice show 1 (Chapter 1, section 1.5; OQ-01, OQ-187; decision batch 11). A roll that was not Pushed never wears anything. No ruling wears an item: a called roll's stakes may lose a carried item, never a point of wear (decision batch 9, 9-5).
- A rule Talent that ignores wear, such as Well-Kept Rig, Blade Discipline, or Sure Seat, removes that point before it applies, so the Pushed roll wears nothing.
...
> **Design note (OQ-01, OQ-187):** a Push rolls a Gear Die showing 2 to 5 again, keeps a 6, and locks a 1, so a Gear Die shows a 1 when the roll is final 27.8% of the time against 16.7% before decision batch 11, and shows a 6 just as much more often. The wear that follows is capped at 1 point per Pushed roll, so a Pushed roll wears an item by 0.278 points on average at rating 1, 0.478 at rating 2, and 0.623 at rating 3 (`tools/probes/round-2/gear_die.out`, exact arithmetic). A higher rating wears faster per Push but has more to lose, and issued ODM Gear rose to rating 3 from Funding 3 up in the same batch so that the Jam test stays under its bar (section 4.9; `data/engagement/tuning.yaml`, `jam_test`). Every figure the batch reads is stale until the simulator rerun that batch schedules.
...
- **Rated** like any rated item: its Gear Dice on a Shoot roll are its current rating, a Pushed roll whose Gear Dice show any 1 wears it by 1 point, and at 0 it is worn out and cannot be fired until Field Repair or Maintain Gear restores it (decision batch 8, 8-12).
...
> **Design note (OQ-25; decision batches 2b and 11):** Standard Issue rated a horse as it rated ODM Gear at every Funding until decision batch 11 raised ODM Gear to 3 from Funding 3 up and left the horse ladder at 1, 1, 2, 2, 2, 3 (section 4.9). From Funding 3 a mounted dodge therefore rolls one Gear Die fewer than an ODM one, and the choice is no longer a choice of cost alone: it is a smaller pool and a cheaper item against a Gas Roll and a Jam. A mounted soldier is never airborne, so a Jam from the saddle drops no one. The figures below were measured under the older parity and are stale until the rerun (section 5.13).
...
| 3 (used until the Funding rules are written) | 3 | one, full | 1 | 3 | 1 | 2 |
| 4 | 3 | one, full | 1 | 4 | 1 | 2 |
| 5 | 3 | one, full | 2 | 4 | 1 | 2 |
