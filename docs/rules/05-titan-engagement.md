...
**Carry limit:** at most 2 Carries on one Flight (provisional, OQ-200). A Flight never ends free in an Open zone, and one that ends free ends Anchored.

**Mounted pace:** up to 2 zone steps and 0 attachment steps (provisional, OQ-200). **On foot:** 1 step.
<!-- END RENDERED: carry-costs -->

- **On foot:** one step, a zone step or an attachment step.
- **Mounted:** up to two zone steps and no attachment step. The move ends on entering a zone that holds a standing Focus Titan, unless that zone is Open, where the rider may ride on through. A move can include one mount or one dismount, before or after its steps (Chapter 4, section 4.5). PROVISIONAL: the mounted pace of two is chosen, not measured (OQ-200).
...
- **Each further step is a Carry,** paid in Momentum: a zone step costs the Carry cost of the zone it enters (Open 2, Sparse 1, Wooded, Urban, and Giant Forest 0), a step off field costs 1, and an attachment step costs 1. A Flight makes **at most 2 Carries**. PROVISIONAL: the limit is chosen, not measured (OQ-200).
...
- the anchors each Anchor Rating gives, its Carry cost, the Carry limit, the mounted pace, what a point of Momentum buys, and the Terrain Traits (section 5.2; OQ-200);
- each Size Class's Stride and the Sprinting Abnormal's (section 5.4);
- the terrain mix of the interim setup table (section 5.1; OQ-202);
...
- **The tracker row** reads `A Medium | zone 7, Stride 0 | cards 9 | Att: Jonas | Next: [hidden] | Prev: Roar | Eyes I0 LA I0 RA I0 LL B0 RL W1 | Op 1 (Jonas) | Regen 2/3 | Decoys 0 | Grab: - | Flags: - | Frenzy 1`. Two rounds have ended, so its Frenzy is 1 (it rises at the end of every even-numbered round) and every behavior roll it makes now is D6 plus 1.
