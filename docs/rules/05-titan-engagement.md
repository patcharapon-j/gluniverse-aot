...
5. Soldiers and horses are placed (`positions.yaml`, `placement`).
...
- **In Reach** is within reach of the Titan's hands, on the ground or on anchors beside it.
- **On Body** is hooked into the Titan's body or standing on it, anywhere but the Nape. It is literally on the thing.
- **Blind Spot** is out of the Titan's sight with its Nape within reach, **anchored to terrain behind the Titan and not on it**. The soldier is anchored to a tree or a roof, hanging from it on their lines or standing on it, and not touching the Titan: it is a place in the world, not a place on the monster. Whether they are airborne follows the move that brought them there, as anywhere: an ODM move leaves them airborne, so a Jam drops them (Chapter 4, section 4.3) and a falling Titan's body misses them (section 5.7); only the Urban Terrain Trait says the roof holds them. It is the only Position a Nape strike can be made from (ADR-0010).

That last clause is worth reading twice, because everything about the Blind Spot follows from it. Steam at a Regeneration fill burns the soldiers On Body and not those at Blind Spot, because Blind Spot is not on it (section 5.7). The Urban Terrain Trait says a Blind Spot soldier is anchored to a roof and is not airborne, because there they are standing on one; at every other rating they hang from their anchor and stay airborne. And whether a Blind Spot exists at all, while the Titan stands, is the terrain's answer and not the Titan's: on Open ground there is nothing to anchor to and there is no Blind Spot (*The three Position maps*, below; decision batch 13, 13-6).

**What changes a Position.** A closed list (`positions.yaml`, `changes_to_position`; decision batch 13, 13-5). It states rules that already hold and changes none of them, and it is headed by the principle that carries the most weight at the table:

> **An action never changes a Position.** Only the soldier's own move changes it, plus the rules listed here.

| What | Effect |
|---|---|
| The soldier's own move | One step the Anchor Rating allows, or a mount, a dismount, or leaving. An ODM move is a Flight |
...
Each rating also gives the Titan Engagement a pool of **Anchors** and one **Terrain Trait**, in the second table below (*Anchors* and *Terrain Traits*).
...
**A grounded Titan changes every map** (`grounded_titan`). Every step joining two of In Reach, On Body, and Blind Spot can then also be made on foot, and none of them mounted, so the shape holds and the ways of making it widen. At the Open rating a grounded Titan also gains an On Body to Blind Spot step, so the Open map becomes the Sparse chain and a Nape strike becomes possible on a plain at last. When the Titan has no Broken leg those permissions end, and a soldier who holds Blind Spot relative to it at the Open rating then holds On Body instead. A corpse counts as a grounded Titan for as long as it lies on the field, and for it they never end (*A corpse*, below).
...
A mounted move that makes the **Distant to In Reach** step may also set the loudest flag on that Focus Titan (`mounted_charge`). Apart from the charge, the only move that sets a flag is a Flight that scores no successes (*Flight*, above). The rider must not be Grabbed or carried. It spends nothing beyond the move, it is never rolled, and it is not ODM use, so it costs no gas.
...
- **Focus Titan row:**
  ...
  - its **Frenzy**, from 0 to 3, raised at the Frenzy end step of every even-numbered round and added to every behavior roll it makes from then on (section 5.4);
...
The GM updates the tracker after the deal, after each Titan card (including the Attention a retarget moved at the Choose step), after each strike or Break Attention, after each counted turn of a Grabbed soldier (the countdown and lifted), after each fall of a Titan's body and each Heave (pins and the heave count), and at the end steps (`per_round_order`). Players track everything else on their own sheets: Positions per Focus Titan, Momentum, mounted, airborne, turns spent in advance, Stress, harm, and gear.
...
**Frenzy** (`frenzy`; decision batch 13, 13-10). Each Focus Titan holds a whole number from 0 to 3. It starts at 0 when the Titan becomes a Focus Titan, rises by 1 at the Frenzy end step of every even-numbered round (section 5.3), and never falls. It is public, and it sits on the Titan's tracker row. It does one thing: it is added to the behavior roll. It adds no dice, changes no Attack Dice, no need, and no effect, and it moves no soldier. Because every table is ordered terrorize to kill by result, adding Frenzy walks a Titan up its own table as the fight drags: it postures for three rounds and is trying to kill from round 4. A Squad that kills in the three rounds the design targets never sees it, and a Squad that grinds meets Frenzy 3 on the round the retreat clock fills. The rate was every round when the rule was first written; the retune halved it, because at every round the Squad fighting exactly as intended paid it too (round 3 retune, R1). The GM never raises, lowers, or spends it (ADR-0024, limit 8).
...
> - **Lone strikers:** a lone striker's Break Attention successes help only the comrade who follows. With them spendable, the lone Rookie would reach 15.9% at Stress 1, above the 8% to 16% band, and a lone soldier's short cut would feed their own next cut, which ADR-0007 rejected when it stopped Nape cuts adding up (section 5.13).
>
...
>
> On the final full simulator rerun under Attack Dice, the Rookie's fresh cut at Stress 1 succeeds 15.0% of the time, inside ADR-0014's band of 8% to 16% (re-read by the round 3 retune, R4, because decision batch 11's Push re-rolls the blade's Gear Dice; 12.9% without that rule), and the Levi-grade soldier's 47.2% at Stress 2 is inside "about 50%", which reads 45% to 55% (ADR-0014, as amended in decision batch 5; `docs/reviews/simulator-report.md`, sections 1 and 10). The probes gave 13.1% and 47.4%. Nape Depth 5 would bring the Rookie to 3.6% but drop the Levi-grade soldier to 28.5% and the prepared-Squad kill to a median of round 5.
>
...
> - **Earlier injuries:** a victim already carrying untreated leg injuries died every time alone at Health 2 or 3, where the crush put them Down, and about 70% of the time at Health 4 to 6; with one comrade, 48% and 33 to 34% (`health_reports`). Those rows were measured before decision batch 13 (13-1; OQ-190) raised Health by 2, so no soldier can now reach the Health 2 and 3 rows and the rerun measures Health 4 to 8 instead.
...
- **The tracker row** reads `A Medium | cards 9 | Att: Jonas | Next: [hidden] | Prev: Roar | Eyes I0 LA I0 RA I0 LL B0 RL W1 | Op 1 (Jonas) | Regen 2/3 | Decoys 0 | Grab: - | Flags: - | Frenzy 1`. Two rounds have ended, so its Frenzy is 1 (it rises at the end of every even-numbered round) and every behavior roll it makes now is D6 plus 1.
...
- **Next.** Shake Off becomes the previous behavior, Mila's flag clears, and the GM rolls the new Next Behavior out of sight, D6 plus the Frenzy of 1 that stands at this moment.
