...
- **Tuning tests:** Attack Dice are keyed to the fixed needs that were tuned against the Rookie dodge, 3 dice per point (decision batch 8, 8-1), and the Jam test and the six Grab cells are shown in section 5.13. The probe figures there were measured with those fixed needs; the confirming run of the zone retune re-measured every verdict under Attack Dice and on the field, and every ADR-0014 target, every Chapter 6 band, and every limit of the Sprinting Abnormal's bar was Met, which closed OQ-199 (section 5.13; Chapter 6, section 6.6; decision batch 8, 8-14 and 8-31; the round 3 retune, R6 and R7; the zone retune, Z5; OQ-145). Every ADR-0014 target is met as amended in decision batches 3, 3b, 3c, 3e, and 5 (OQ-79, OQ-81, OQ-95, OQ-98, OQ-127), each under its baseline policy and read with batch 5's tolerances; ADR-0014, as amended in decision batch 4b, reads the prepared-Squad target under the baseline Squad, with the four-striker row reported beside it (section 5.13; OQ-112).
...
> **Design note (OQ-75; decision batch 16, 16-7):** Everyone starts one zone out. A Titan Engagement opens when Titans are sighted, not when they are already in reach, and one zone away keeps the two moves to the Nape that Distant always meant. On the Skirmish and Standard fields the start zone is an edge zone, so a soldier who starts there can leave with one move, as a soldier at Distant could; on the Set piece it is one zone in from the edge, so leaving takes two. A Titan with a Stride of 2 closes on its first card, so round 1 is not what it was before zones; the rerun reads round 1 on its own (section 5.13).
...
So "can a soldier go from In Reach to Blind Spot without going On Body first?" is answered **yes** in a Wooded, Urban, or Giant Forest zone, **no** in a Sparse zone, and **there is no Blind Spot** in an Open zone. Terrain decides whether the Squad can reach the Nape without climbing the thing, which is why the Corps fights in forests, and on a mixed field it decides where along the way.
...
- **Cap:** the anchors of the zone the soldier is in. When a soldier's zone changes, or its rating falls, a soldier whose Momentum is above the new cap loses the excess at once, except during a Flight: a Flight's zone steps trim nothing, and its Momentum is trimmed only at the end of the move, to the anchors of the zone it ends in.
...
- **A zone that becomes Open:** every soldier Anchored in that zone lands Ground and stops being airborne, with no fall, whatever stands in it (PROVISIONAL: OQ-205). When a standing Focus Titan is in it, every soldier who holds Blind Spot relative to that Titan also holds On Body instead, a change made by this rule and not a fall (`anchor-ratings.yaml`, `zone_becomes_open`; decision batch 16, 16-21).
...
Every fall in a Titan Engagement reads one **reference body**: the body the soldier's attachment named when they fell (On Body, Blind Spot, or Grabbed); otherwise the Focus Titan whose card, Grab, or effect caused the fall; otherwise the Focus Titan nearest the soldier's zone, a tie going to the earliest label; otherwise, with no Focus Titan alive, the corpse nearest the soldier's zone, a tie going to the earliest label. With no body at all the fall has no reference body, and its band starts low before any raise (PROVISIONAL: OQ-206) (Chapter 4, section 4.6; decision batch 5, OQ-123; decision batch 16, 16-22). The band reads the soldier's Position relative to it, and the Large Size Class raise reads its class; a Giant Forest raise reads the zone the soldier falls in. A soldier who falls **stays in the zone they are in**, which for an attached soldier is the body's zone, and lands **Ground**, so they then hold In Reach relative to that body (`falls_land`). A fall is never the soldier's own move.
...
- It enters at the **entry zone**: the edge zone that holds no soldier and is farthest from the nearest soldier on the field, a tie going to the lowest-numbered; if every edge zone holds a soldier, the edge zone holding the fewest, then the lowest-numbered; with no soldier on the field, the lowest-numbered edge zone (PROVISIONAL: OQ-207) (`zones.yaml`, `entry_zone`; decision batch 16, 16-29). A Background Titan has no zone until then.
...
**Why a retreat always ends** (`retreat`, `termination`; decision batch 16, 16-27). A soldier bound by the retreat who takes option 1 or 2 on each of their moves leaves within five of them: at most two to become free, at most two to reach an edge zone, and one off field. A Stride never lengthens this, because it frees a soldier at Blind Spot, carries only soldiers who are not yet free, and never moves a free soldier; a Grab stops the soldier's moves as it always has, and its release leaves them free. The stay limit bounds options 3 and 4 as it always has.
...
> **Measured on the field (decision batch 16, 16-38; the zone retune, Z5).** Every figure, verdict, target reading, band, and bar limit printed in this section is the confirming run of the zone retune, which measured the rules as they now stand, zones and the Stride included: every ADR-0014 target, every per-table band, and every limit of the Sprinting Abnormal's bar is Met, and the run reads no missed result (`docs/reviews/simulator-report.md`, Summary and sections 1, 3, and 5). At the standard Medium Titan's reference start on the field (section 7.2 of the report) the median kill comes in round 3 and the median first Nape strike in round 2; round 1 alone deals 0.23 Critical Injuries and lands 0.56 of the Titan's cards; 7.3% of its resolved cards resolve Thrash and 33.4% retarget; its Stride brings the holder into its zone on 7.5% of them; a fight makes 2.67 Flights, 56.3% of them with a Carry; Momentum gained is 2.46 a fight, of which 1.63 is spent on Carry, none on quiet, and 0.35 on bite; 64.2% of the Help on Nape strikes comes from an adjacent zone; a fight wrecks 1.09 rating steps, and the Titan's zone loses its Blind Spot while it stands in 0.7% of fights; 4.8% of fights end in a retreat, 0.24 retreat rounds a fight. Every Stride above 0 reads the same (Stride 1 and Stride 3 give 0.142 and 0.136 deaths through the end against 0.137), and Stride 0 gives 0.75 Critical Injuries against 0.83. The Carry limit, the mounted pace, and the terrain mix move the deaths through the end by at most 0.017 (a Carry limit of 1), so the values OQ-200 and OQ-202 hold stand (Z1, Z4). Attack Dice, Nape Depth, and Tempo stay in reserve.
...
**Measured together on the confirming run of the zone retune.** Zones, the Stride, and derived Positions (decision batch 16; ADR-0029), Flight, Momentum, Anchors and the `wreck` effect, the loudest flag's match on the ladder, the mounted charge, the blade swap that spends the move, and the Flight a retreat's own move still is (decision batch 10, OQ-182 to OQ-186), decision batch 11's Gear Die re-roll on the Push with its wear cap and its issued ODM Gear rating (OQ-187), decision batch 13's Health and retargeting (OQ-190, OQ-192), Frenzy at every third round (OQ-193; the zone retune, Z2), and the Sprinting Abnormal's Pitch Headlong without its wreck (Z3) were all measured on one run, the confirming run of the zone retune (`docs/reviews/zone-retune-decisions.md`, Z5; rules hash `54e147e73e8d0852`, and the report it rendered is dated 2026-09-21). Every figure, target, band, and bar limit printed in this section is that run's, as `docs/reviews/simulator-report.md` renders it. The probe figures in the tables below were measured before those rules and stand as the record of the rules they measured.
...
> - **The band's measurement point:** the fresh cut above is the band (15.0% on the confirming run). The in-fight cut comes after the Roars a waiting soldier takes, and on the confirming run succeeds 22.0% of the time at a mean Stress of 3.10 (the probes: 14.1% at 2.66, and 12.2% from fight-start Stress 0). It is reported beside the band and not tuned (`docs/reviews/simulator-report.md`, section 2.2).
...
>
> In the probes, alone, a victim whose dodge failed dies 69.9% of the time, and one who did not dodge 73.1%. A victim who always dodges a Grab dies 55.5% of the time per Grab card, so dodging is the better play. Template victims alone die 69.5% (Brawler) to 76.3% (Flier and Hunter); on the confirming run, 67.5% (Brawler) to 74.3% (Hunter) (`docs/reviews/simulator-report.md`, section 2.3). A victim whose turn this round was already spent in advance dies 70.1% after a failed dodge and 73.0% without one; before the failed dodge's turn was given back, that victim died every time after a failed dodge (`turn_debt`).
>
...
- Steam: Mila and Jonas are at Blind Spot relative to A, so each rolls D6 once on the steam table and takes the damage its row gives. Ilse is not, and takes nothing.
- A was already grounded, so its body does not fall a second time.
