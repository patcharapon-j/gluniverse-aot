...
| Swap Blade Set | Option of the soldier's own turn in a Titan Engagement; outside one, no turn (data/gear/blade-sets.yaml, swap). data/gear/blade-sets.yaml (swap). | none | fit a carried Blade Set into empty handles | none | In a Titan Engagement it spends the soldier's move and never the action (decision batch 10, OQ-183; data/gear/blade-sets.yaml, swap). Outside one it spends nothing. | Chapter 4 |
